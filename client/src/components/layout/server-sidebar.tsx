...
// ── Nav button with Discord-style squircle → pill animation ──────────────────
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="relative flex items-center">
            ...
            />
            {/* Hover pip shown when there's an unread notification on an inactive server */}
            {!active && notify && (
            ...
            >
              {children}
              {/* Unread badge on inactive server */}
              {notify && !active && (
...
// Abbreviate a server name to 1–2 initials for the fallback icon
...
export default function ServerSidebar() {
  ...
  // Triggers onQueryStarted which seeds s.server.servers via setServers dispatch
  useGetMyServersQuery();

  // s.server.servers is populated by server_api.ts onQueryStarted
  // s.server.activeServerId (renamed from activeServer in server_slice fix #23)
