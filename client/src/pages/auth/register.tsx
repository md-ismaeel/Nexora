    ...
    <div className="mt-2 space-y-1.5">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
        {RULES.map((rule) => {
          ...
          return (
            <span key={rule.label} className={cn("flex items-center gap-1 text-[11px] transition-colors", ok ? "text-green-400" : "text-[#4e5058]")}>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <Label className="text-xs font-bold uppercase tracking-wide text-[#b5bac1]">
        ...
        {!required && <span className="ml-1 text-[11px] normal-case font-normal text-[#4e5058]">(optional)</span>}
      </Label>
      {children}
...
export default function Register() {
  ...
  // FIX: renamed from `register` to `registerUser` — the original name shadowed
  // react-hook-form's `register` method, requiring the workaround alias `field`.
  // Now both can be used naturally without aliasing.
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      // RTK Query stores the error in the `error` binding above
    }
  };
  ...
  return (
    <div className="w-full rounded-lg bg-[#313338] shadow-2xl">
      <div className="p-8 scrollbar-thin scrollbar-thumb-[#1e1f22]">
        {/* Heading */}
        <div className="mb-6 text-center">
          <h1 className="text-2xl font-bold text-white">Create an account</h1>
          <p className="mt-1 text-sm text-[#b5bac1]">Join the community today — it's free!</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <Field label="Display Name" required error={errors.name?.message}>
            <Input {...register("name")} autoFocus placeholder="John Doe"
              className={cn("border-none bg-[#1e1f22] text-white placeholder:text-[#4e5058] focus-visible:ring-2 focus-visible:ring-[#5865f2]", errors.name && "ring-2 ring-[#ed4245]")} />
          </Field>

          {/* Email */}
          <Field label="Email" required error={errors.email?.message}>
            <Input {...register("email")} type="email" autoComplete="email" placeholder="you@example.com"
              className={cn("border-none bg-[#1e1f22] text-white placeholder:text-[#4e5058] focus-visible:ring-2 focus-visible:ring-[#5865f2]", errors.email && "ring-2 ring-[#ed4245]")} />
          </Field>

          {/* Username */}
          <Field label="Username" error={errors.username?.message} hint="Lowercase letters, numbers, underscores and dots only">
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-[#4e5058]">@</span>
              <Input {...register("username")} placeholder="john_doe"
                className={cn("border-none bg-[#1e1f22] pl-7 text-white placeholder:text-[#4e5058] focus-visible:ring-2 focus-visible:ring-[#5865f2]", errors.username && "ring-2 ring-[#ed4245]")} />
            </div>
          </Field>

          {/* Phone number */}
          <Field label="Phone Number" error={errors.phoneNumber?.message} hint="Include country code, e.g. +919999999999">
            <Input {...register("phoneNumber")} type="tel" placeholder="+919999999999"
              className={cn("border-none bg-[#1e1f22] text-white placeholder:text-[#4e5058] focus-visible:ring-2 focus-visible:ring-[#5865f2]", errors.phoneNumber && "ring-2 ring-[#ed4245]")} />
          </Field>
          ...
          <Field label="Password" required error={errors.password?.message}>
            <div className="relative">
              <Input {...register("password")} type={showPw ? "text" : "password"} autoComplete="new-password"
                className={cn("border-none bg-[#1e1f22] pr-10 text-white placeholder:text-[#4e5058] focus-visible:ring-2 focus-visible:ring-[#5865f2]", errors.password && "ring-2 ring-[#ed4245]")} />
          ...
          <Field label="Confirm Password" required error={errors.confirmPassword?.message}>
            <div className="relative">
              <Input {...register("confirmPassword")} type={showCPw ? "text" : "password"} autoComplete="new-password"
                className={cn("border-none bg-[#1e1f22] pr-10 text-white placeholder:text-[#4e5058] focus-visible:ring-2 focus-visible:ring-[#5865f2]", errors.confirmPassword && "ring-2 ring-[#ed4245]")} />
          ...
          {/* Terms — TODO: replace spans with real Link components when ToS page exists */}
