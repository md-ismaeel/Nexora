...
export default function VerifyEmailPage() {
  ...
  const [verify, { isLoading: verifying, error: verifyError }] =
    useVerifyEmailOtpMutation();
  const [resend, { isLoading: resending, isSuccess: resendSuccess }] =
    useSendEmailOtpMutation();
  ...
  // Start cooldown on mount (OTP was sent on register)
  useEffect(() => {
          ...
          {otp.map((digit, i) => (
            <input
              ...
              className={cn(
                ...
                digit
                  ? "border-[#5865f2]"
                  : "border-[#35363c] focus:border-[#5865f2]",
              )}
            />
          ))}
        </div>
