    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-[#313338] px-4">
            <div className="w-full max-w-md rounded-lg bg-[#2b2d31] p-8 text-center shadow-xl">
                ...
                {sendSuccess && (
                    <div className="mt-4 rounded-md border border-green-500/20 bg-green-500/10 px-3 py-2 text-sm text-green-400">Code sent to your phone.</div>
                )}

                {/* Request code first */}
                {cooldown === 0 && !sendSuccess && (
                    <Button onClick={handleSendOtp} disabled={sending} className="mt-6 h-11 w-full bg-[#5865f2] text-base font-medium text-white hover:bg-[#4752c4] disabled:opacity-60">
                        {sending ? (
                ...
                {(sendSuccess || cooldown > 0) && (
                    <>
                        ...
                        <Button onClick={handleSubmit} disabled={verifying || code.length < 6}
