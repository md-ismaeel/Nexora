...
import { useGetDmHistoryQuery, useSendDmMutation, useMarkDmReadMutation } from "@/api/dm_api";
...
// ── Motion
import { motion, AnimatePresence, Messages as MessageMotion, makeStagger } from "@/lib/motion";
// ── Icons
...
function Bubble({
    ...
    msg: IDirectMessage; isOwn: boolean; showAvatar: boolean;
    recipientName: string; recipientAvatar?: string;
...
function ConversationStart({ name, avatar }: { name: string; avatar?: string }) {
    return (
        <motion.div
        ...
        >
            ...
            <div>
                ...
                <p className="mt-1 text-sm text-[#949ba4]">
                    This is the beginning of your direct message history with <strong className="text-white">{name}</strong>.
                </p>
            </div>
        </motion.div>
    );
...
export default function DirectMessagePage() {
    ...
    useEffect(() => {
        if (userId) { markRead(userId); dispatch(clearUnread(userId)); }
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto">
                {isLoading ? (
                    ...
                    <motion.div variants={makeStagger({ staggerChildren: 0.02 })} initial="hidden" animate="visible">
                        ...
                        <AnimatePresence>
                            {messages.map((msg, i) => {
                                ...
                                return (
                                    <Bubble
                                        key={msg._id} msg={msg} isOwn={isOwn} showAvatar={!grouped}
