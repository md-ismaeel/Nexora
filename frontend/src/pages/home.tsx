    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <nav className="flex items-center justify-between px-8 py-4 border-b border-border bg-card/60 backdrop-blur-md sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <Chip
            size="sm"
            classNames={{
              base: "bg-primary/10 border border-primary/30 h-5",
              content: "text-primary text-[10px] font-semibold px-1",
            }}
          >
            BETA
          </Chip>
          ...
          <div className="flex items-center gap-3 mt-2">
            <div className="flex -space-x-2">
              {ONLINE_MEMBERS.map((m) => (
                <Tooltip key={m.name} content={m.name} placement="top">
                  <div
        ...
        <motion.div {...vp(Primitives.fade)} className="flex flex-col gap-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {FEATURES.map((f) => (
              <Card
                key={f.title}
                className="bg-card border border-border hover:border-primary/40 transition-colors group"
              >
      ...
      <section className="px-8 pb-20 max-w-5xl mx-auto w-full">
        <Card className="bg-primary/8 border border-primary/25">
          <Card.Content className="px-10 py-10 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
            ...
            <Button
              size="lg"
              className="bg-primary text-primary-foreground hover:bg-primary/90 font-bold shrink-0 px-8"
              onPress={() => navigate("/channels/@me")}
            >
              Open Ember
            </Button>
